...
const SuccessComponentGraph = ({
  ...
  const results = data.filter((item) => item.status === ANSWER_STATUS.CORRECT);
  const pass = results.length / data.length > 0.7;
  ...
  const resetData = () => {
    ...
    setStateResult(dataDefault.game_config.data);
  };
...
export default SuccessComponentGraph;
